...
send_telegram "🚀 GitHub Actions started. Trying to create VM.Standard.A1.Flex..."

echo "[$(date '+%T')] === GitHub Actions run started ==="
...
⏱️ Attempt: ${ATTEMPT} | Time elapsed: $(( ELAPSED / 60 )) minutes"
  ...
  if ! grep -q "Out of host capacity\|The operation was canceled" /tmp/tf_output.txt; then
    ...
    send_telegram "❌ GitHub Actions — failed due to unexpected error:
${ERROR}"
    exit 1
  fi
