...
send_telegram "🚀 GitHub Actions התחיל לרוץ — מנסה להקים VM.Standard.A1.Flex"

echo "[$(date '+%T')] === ריצת GitHub Actions התחילה ==="
...
while true; do
  ...
  if [ $EXIT_CODE -eq 0 ]; then
    ...
    MSG="✅ Oracle Instance הוקם בהצלחה!
...
⏱️ ניסיון: ${ATTEMPT} | זמן: $(( ELAPSED / 60 )) דקות"
  ...
  if ! grep -q "Out of host capacity\|The operation was canceled" /tmp/tf_output.txt; then
    ...
    send_telegram "❌ GitHub Actions — נכשל מסיבה אחרת:
${ERROR}"
    exit 1
  fi
